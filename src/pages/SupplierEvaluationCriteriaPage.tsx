...
export default function SupplierEvaluationCriteriaPage() {
  ...
  const totalWeight = (Array.isArray(criteria) ? criteria : []).filter(c => c.is_active).reduce((sum, c) => sum + c.weight, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{(Array.isArray(criteria) ? criteria : []).filter(c => c.is_active).length}</div>
