...
export async function getSupplierTrainingsForPortal(supplierId: string, companyId: string) {
  ...
  // Get trainings linked to those categories
  const { data: trainingLinks, error: linkError } = await supabase
  ...
  const trainingIds = trainingLinks?.map(t => t.training_material_id) || [];
...
export async function updateTrainingProgress(
  ...
  if (status === 'Em Andamento' && !updateData.started_at) {
    updateData.started_at = new Date().toISOString();
  }
