-- Correções Emergenciais do Motor GEE - Fase 1
...
WHERE name = 'Eletricidade - Sistema Interligado Nacional (SIN) 2025'
AND co2_factor = 61.7;
...
AND co2_factor > 100; -- Só atualiza se tem erro de unidade
...
-- Baseado nas práticas padrão do mercado brasileiro

-- Gasolina Comum (essencial para frotas)
INSERT INTO emission_factors (name, category, activity_unit, co2_factor, ch4_factor, n2o_factor, source, year_of_validity, type)
VALUES 
('Gasolina Comum', 'Combustão Móvel', 'Litro', 2.27, 0.000095, 0.000084, 'PB GHG Protocol / ANP (2025)', 2025, 'system')
ON CONFLICT DO NOTHING;

-- Gás Natural (combustão estacionária)
INSERT INTO emission_factors (name, category, activity_unit, co2_factor, ch4_factor, n2o_factor, source, year_of_validity, type)
VALUES 
('Gás Natural', 'Combustão Estacionária', 'm³', 1.95, 0.000037, 0.000004, 'MCTI (2025)', 2025, 'system')
ON CONFLICT DO NOTHING;
...
WHERE name IN ('Energia Elétrica - SIN', 'Etanol Hidratado', 'Óleo Diesel Rodoviário (S10)');