-- Soft-delete support for laia_assessments
-- Adds a deleted_at column so accidental deletions can be restored,
-- plus partial indexes for active vs. trashed reads.
-- RPCs get_laia_branch_stats / get_laia_dashboard_stats are updated
-- to exclude soft-deleted rows.

ALTER TABLE public.laia_assessments
  ADD COLUMN IF NOT EXISTS deleted_at timestamptz NULL;

CREATE INDEX IF NOT EXISTS laia_assessments_active_branch_idx
  ON public.laia_assessments (company_id, branch_id)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS laia_assessments_deleted_idx
  ON public.laia_assessments (company_id, branch_id, deleted_at DESC)
  WHERE deleted_at IS NOT NULL;

CREATE OR REPLACE FUNCTION public.get_laia_branch_stats(p_company_id uuid)
 RETURNS TABLE(branch_id uuid, total bigint, criticos bigint, significativos bigint, nao_significativos bigint)
 LANGUAGE sql
...
AS $function$
  ...
  WHERE a.company_id = p_company_id AND a.status = 'ativo' AND a.deleted_at IS NULL
  GROUP BY a.branch_id;
$function$;

CREATE OR REPLACE FUNCTION public.get_laia_dashboard_stats(p_company_id uuid, p_branch_id uuid DEFAULT NULL::uuid)
 RETURNS json
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
  ...
  SELECT json_build_object(
    ...
    'by_sector', COALESCE((
      ...
      FROM (
        ...
        WHERE la.company_id = p_company_id AND la.status = 'ativo' AND la.deleted_at IS NULL
    ...
    'by_temporality', COALESCE((
      ...
      FROM (
        ...
        WHERE company_id = p_company_id AND status = 'ativo' AND deleted_at IS NULL
